# Shizuku (Modded Version)

> [!NOTE]
> **Pemberitahuan Versi Mod / Disclaimer**:
> Ini adalah repositori **versi modifikasi (Modded / Forked Version)** dari proyek resmi [Shizuku](https://github.com/RikkaApps/Shizuku) karya [RikkaApps](https://github.com/RikkaApps).  
> Seluruh hak cipta, merek dagang, dan arsitektur dasar merupakan milik pengembang aslinya di bawah lisensi Apache 2.0 & MIT. Modifikasi ini dibuat untuk keperluan personal, perbaikan tata letak UI/UX, dan penambahan fitur akses file internal.

---

## 🌟 Fitur & Perubahan di Versi Mod (Mod Features)

Dibandingkan dengan versi resmi Shizuku, versi modifikasi ini memiliki berbagai peningkatan dan penyesuaian:

1. 📂 **Built-in `Android/data` & `Android/obb` Explorer**:
   * Akses langsung untuk melihat, membaca, membuat, menyalin, dan menghapus file di direktori terbatas `Android/data` dan `Android/obb` menggunakan layanan Shell ADB tanpa perlu akses Root.
2. 🔄 **Tombol Dinamis Mulai / Hentikan (Start/Stop Switch)**:
   * Tombol aksi utama di layar Home berganti otomatis secara *real-time*:
     * Saat belum aktif: Tombol **Mulai (Start)** berwarna primer.
     * Saat sudah aktif: Tombol **Hentikan (Stop)** dengan aksen peringatan merah dan konfirmasi penghentian layanan.
   * Menu titik tiga (overflow menu) dibersihkan dari opsi stop ganda agar tampilan lebih ringkas.
3. 🎨 **Perombakan UI/UX Home yang Bersih & Modern**:
   * *Hero Status Card* baru bergaya Material 3 dengan indikator status yang jelas.
   * Tata letak tombol berdampingan (*side-by-side*) antara Pairing dan Mulai.
   * Menghapus kartu-kartu berlebih yang jarang digunakan agar antarmuka fokus dan ringan.
4. 📖 **Tata Letak Panduan Pairing Baru & Bahasa Santai**:
   * Halaman pairing dirapikan dengan pengelompokan khusus: **Bantuan & Tips** (Wi-Fi & optimasi baterai MIUI/HyperOS) dan **Langkah Pairing 1, 2, 3**.
   * Seluruh kalimat bahasa Indonesia diperbarui menjadi bahasa sehari-hari yang santai, lugas, dan nyaman dibaca.
5. 🌐 **Pemilihan Bahasa Langsung di Aplikasi**:
   * Fitur ganti bahasa langsung (Bahasa Indonesia & English) melalui menu aplikasi, dengan bahasa Inggris sebagai *default*.
6. 📘 **Buku Panduan Native Offline**:
   * Menggantikan panduan web eksternal menjadi halaman panduan native yang bisa dibaca tanpa koneksi internet.
